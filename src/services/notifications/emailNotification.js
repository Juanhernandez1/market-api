...
async function sendCodeForVerifyEmail(transport, data) {
  let info = await transport.sendMail({
    ...
    text: "Hola  " + data.name + " !",
    html:
      "<p>Por favor verifica tu cuenta con el siguiente codigo:  <b>" +
      data.code_to_verify_email +
      "</b></p>"
  });
...
async function sendLinkForResetPassword(transport, data) {
  ...
  let info = await transport.sendMail({
    ...
    text: "Hola  " + data.name + " !",
    html:
      "<p>Por favor has clic sobre el siguiente link, para cambiar tu contraseña: " +
      "</p> " +
      "<a href='" +
      config.app.host +
