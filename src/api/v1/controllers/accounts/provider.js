...
function providerAccounts(router) {
  ...
  router.post(
    "/accounts/providers/sing-up",
    signUpProviderValidationRules(),
    validate,
  ...
  router.put(
    "/accounts/providers/profiles/update/:user_id",
    updateProfileProvidersValidationRules(),
    [authVerify, isProvider, validate],
    async function (req, res) {
      const { body: user } = req;
